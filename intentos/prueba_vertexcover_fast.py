...
#G = nx.read_weighted_edgelist("aviones.edges", nodetype=int)
...
MAX_FITNESS = (G.order() + 20) ** 2 # Tal y como está definido, el máximo valor que puede tomar el fitness es el número de nodos del grafo
MAX_ITER = 500
SAMPLE_SIZE = 10
...
for subset in r[1:]:
    ...
    if is_cover(elem, G):
        print(elem)
        print(subset)
        print(num_ceros - len(subset), "ceros")
        break
    for index in subset:
        elem[index] = 0